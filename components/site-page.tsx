...
function MemeStrip() {
  const memes = ["/public%3Ameme-1.png", "/public-meme-2.png", "/public%3Ameme-3.png", "/public-meme-4.png", "/public-meme-5.png"];
...
function Token() {
  ...
  return <section id="token" className="bc-token live-token"><div className="bc-section-head"><span>TOKEN</span><p>BeanCat on Arc.</p></div><div className="live-token-card"><div className="live-token-main"><span>BEANCAT / ARC</span><h2>ARCHIVED LORE.<br/><strong>LIVE ON-CHAIN.</strong></h2><p>A community meme token inspired by the archived cat behind the historical @arc profile.</p><div className="bc-token-actions"><ActionLink href={tokenConfig.buyUrl} className="bc-primary">BUY 豆 <ArrowUpRight size={16}/></ActionLink><button className="bc-outline" onClick={copy}>{copied?<Check size={15}/>:<Copy size={15}/>} {copied?"COPIED":"COPY CA"}</button><ActionLink href={tokenConfig.explorerUrl} className="bc-outline">ARC SCAN <ArrowUpRight size={14}/></ActionLink></div></div><div className="live-token-side"><div><small>NETWORK</small><strong>{tokenConfig.network}</strong></div><div><small>TICKER</small><strong>{tokenConfig.ticker}</strong></div><div><small>CONTRACT</small><code>{tokenConfig.contract}</code></div></div></div></section>;
}
