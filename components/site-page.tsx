...
function MemeImage({ src, align = "center" }: { src: string; align?: "left" | "center" | "right" }) {
  return <div className={`bc-meme-inline bc-meme-${align}`}><motion.div className="bc-meme-frame" initial={{opacity:0,y:10}} whileInView={{opacity:1,y:0}} viewport={{once:true,amount:.2}} transition={{duration:.35}}><img src={src} alt="BeanCat community meme" draggable={false}/></motion.div></div>;
}
...
function Proof() {
  return <section id="proof" className="bc-proof live-section">
    <div className="bc-proof-inner">
      ...
      <div className="bc-proof-stack"><ActionLink href={tokenConfig.archiveUrl} className="bc-browser"><div className="bc-browser-bar"><i/><i/><i/><code>web.archive.org / twitter.com/arc / 20150823022308</code></div><img src="/archive-proof.png" alt="Wayback Machine evidence" draggable={false}/><div className="bc-browser-caption"><span>PUBLIC ARCHIVE</span><strong>@arc · 豆 · CAT · 2015</strong></div></ActionLink><MemeImage src="/public:meme-1.png" align="right"/></div>
    </div>
  </section>;
}
...
function Token() {
  ...
  return <section id="token" className="bc-token live-token">
    ...
    <div className="live-token-card">
      <div className="live-token-main"><span>BEANCAT / ARC</span><h2>ARCHIVED LORE.<br/><strong>LIVE ON-CHAIN.</strong></h2><p>A community meme token inspired by the archived cat behind the historical @arc profile.</p><MemeImage src="/public:meme-2.png" align="left"/><div className="bc-token-actions"><ActionLink href={tokenConfig.buyUrl} className="bc-primary">BUY 豆 <ArrowUpRight size={16}/></ActionLink><button className="bc-outline" onClick={copy}>{copied?<Check size={15}/>:<Copy size={15}/>} {copied?"COPIED":"COPY CA"}</button><ActionLink href={tokenConfig.explorerUrl} className="bc-outline">ARC SCAN <ArrowUpRight size={14}/></ActionLink></div></div>
...
function Community() {
  return <section id="community" className="bc-community live-community"><div className="bc-community-inner"><span className="bc-light-label">COMMUNITY</span><div className="live-community-grid"><div><h2>BEANCAT<br/><strong>IS LIVE.</strong></h2><MemeImage src="/public:meme-3.png" align="left"/></div><div className="bc-community-copy"><p>Follow the project and join the community.</p><div className="bc-community-actions"><ActionLink href={tokenConfig.xUrl} className="bc-community-button">FOLLOW X <ArrowUpRight size={15}/></ActionLink><ActionLink href={tokenConfig.telegramUrl} className="bc-community-button">JOIN TELEGRAM <ArrowUpRight size={15}/></ActionLink><ActionLink href={tokenConfig.buyUrl} className="bc-community-buy">BUY 豆 <ArrowUpRight size={15}/></ActionLink></div></div></div></div></section>;
}
