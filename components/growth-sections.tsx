...
export function MarketStrip() {
  ...
  return (
    <section className="market-strip market-strip-polished" aria-label="Live market data">
      <div className="market-strip-inner">
        <div className="market-live-label"><i/><span>LIVE MARKET</span><small>{loading ? "SYNCING" : updatedAt ? `UPDATED ${updatedAt}` : "DATA UNAVAILABLE"}</small></div>
        <div className="market-cells">{cells.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div>
        <div className="market-actions"><button onClick={load} aria-label="Refresh market data"><RefreshCw size={13} className={loading ? "animate-spin" : ""}/></button><a href={tokenConfig.explorerUrl} target="_blank" rel="noopener noreferrer"><span className="market-source">ARC SCAN</span><ArrowUpRight size={13}/></a></div>
      </div>
      <div className="market-source-line">SOURCE / {data.source ?? "ARC SCAN"} · AUTO REFRESH / 60S</div>
    </section>
  );
}
...
export function ContractBar() {
  ...
  return (
    <section className="contract-quickbar contract-quickbar-sticky">
  ...
  return (
    <section className="section-shell archive-now-section">
      ...
      <Reveal className="archive-now-grid">
        <div className="era-card era-card-past"><div className="era-top"><span>2015</span><small>WEB ARCHIVE</small></div><div className="era-main"><span>@arc</span><strong>豆</strong><p>CAT PROFILE</p></div><a href={tokenConfig.archiveUrl} target="_blank" rel="noopener noreferrer">VIEW RECEIPT <ArrowUpRight size={14}/></a></div>
        <div className="era-bridge"><span>THEN</span><ArrowRight/><strong>NOW</strong></div>
        <div className="era-card era-card-now"><div className="era-top"><span>NOW</span><small>ARC NETWORK</small></div><div className="era-main"><span>Arc</span><strong>豆</strong><p>ON-CHAIN</p></div><a href={tokenConfig.explorerUrl} target="_blank" rel="noopener noreferrer">VIEW ON-CHAIN <ArrowUpRight size={14}/></a></div>
      </Reveal>
  ...
  return (
    <section className="revival-section">
      ...
      <Reveal className="relative mx-auto grid max-w-[1440px] gap-12 lg:grid-cols-[.75fr_1.25fr] lg:items-end">
        ...
        <div className="revival-copy"><strong className="revival-punchline">The archive existed. The community gave it a voice.</strong><p>The original-name 豆 had the archive story, but no active community pushing it forward. The community is bringing that OG narrative back into view — preserving the receipt, the cat and the on-chain token without rewriting the history.</p><div className="revival-facts"><span>ARCHIVE PRESERVED</span><span>COMMUNITY LED</span><span>ON ARC</span></div></div>
      </Reveal>
    </section>
  );
}
  ...
  return (
    <section className="share-cta">
      ...
      <Reveal className="relative mx-auto max-w-[1440px]">
        ...
        <div className="share-actions"><a className="button button-dark" href={xIntent} target="_blank" rel="noopener noreferrer">SHARE ON X <ArrowUpRight size={15}/></a><a className="button button-outline" href={tokenConfig.telegramUrl} target="_blank" rel="noopener noreferrer">JOIN TELEGRAM <ArrowUpRight size={15}/></a><button className="button button-outline" onClick={copy}>{copied ? <Check size={15}/> : <Copy size={15}/>} {copied ? "COPIED" : "COPY CA"}</button></div>
      </Reveal>
    </section>
  );
}
